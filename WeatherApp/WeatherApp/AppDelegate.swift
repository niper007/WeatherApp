...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let searchCityTableViewController = storyboard.instantiateViewController(withIdentifier: "searchCityTableViewController") as! SearchCityTableViewController
        
        let userDefault = UserDefault()
        print("\(userDefault.userAlreadyExist() ?userDefault.getValues().0 : "")")
        
        if(userDefault.userAlreadyExist()){
             self.window?.rootViewController = weatherViewController
        }else{
             self.window?.rootViewController = searchCityTableViewController
        }
